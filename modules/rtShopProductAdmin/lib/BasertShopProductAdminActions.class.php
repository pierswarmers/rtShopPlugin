...
class BasertShopProductAdminActions extends sfActions
{
  ...
  public function executeProductSelect(sfWebRequest $request)
  {
    ...
    $query = Doctrine::getTable('rtIndex')->getBaseSearchQuery($request->getParameter('term'), $this->getUser()->getCulture());
    ...
    {
      $products[] = array('label' => (string) $product, 'id' => $product->getModelId(), 'value' => (string) $product);
    }
