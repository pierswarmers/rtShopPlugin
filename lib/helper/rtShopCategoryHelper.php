 ...
 * @return string
 */
function rt_shop_category_map($rt_shop_category = null, $options = array())
{
  ...
  {
    $string .= rt_shop_category_map_li($root_page, $rt_shop_category, array('closing_tag' => ''));
  }

  $tree = Doctrine::getTable('rtShopCategory')->getDescendantsOfRoot($root_page, null, true);

  if(!$options['render_full'] && !is_null($rt_shop_category))
  {
    $ancestors = $rt_shop_category->getNode()->getAncestors();
    ...
    $cleaned_ancestors[] = $rt_shop_category;
  ...
  if($tree)
  {
    foreach($tree as $node)
    {
      ...
      if(!$options['render_full'] && !is_null($rt_shop_category))
      {
        ...
        if($node['level'] > $rt_shop_category['level'] + 1)
        {
      ...
      $string .= rt_shop_category_map_li($node, $rt_shop_category, array('closing_tag' => ''));;
 ...
 * Print a single 
 */
function rt_shop_category_map_li($page, $selected_page = null, $options = array())
{
  ...
  if($page['level'] == 0)
  {
    $link = link_to($title, 'rt_shop_category_index');
  }
  else
  {
    if(!$page instanceof rtShopCategory)
    {
    ...
    $link = link_to($title, 'rt_shop_category_show', $page);
  }
