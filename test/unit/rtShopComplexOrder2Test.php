 ...
 *
 * $ cp plugins/rtShopPlugin/test/unit/rtShopComplexOrder2Test.php test/unit/rtShopComplexOrder2Test.php
 *
 */

/**
 * rtShopComplexOrder2 Testing - Order testing
 *
...
// Add data
rtShopComplexOrder2TestTools::clean();
...
$tools = new rtShopComplexOrder2TestTools;
...
// Add cart promotion
$cartpromo1 = $tools->createCartPromotion('Cart Promotion 10%', 10, 200, 300, NULL, NULL, 'percentageOff', 1, false);
...
$cm->addToCart($stock3, 1);
$cm->addToCart($stock4, 1);
$cm->addToCart($stock5, 1);
$cm->addToCart($stock6, 1);
$cm->addToCart($stock7, 1);
...
$i=0;
$compare_charge = array(40,30,36,36,27,30,36);
foreach($stock_info as $stock)
{
  ...
  if($prod_promo)
  {
    ...
    $message .= " || RedType: ".$prod_promo->getReductionType();
    $message .= " || RedValue: ".$prod_promo->getReductionValue();
  }
...
$t->comment('*****************************************************************************');
// ItemsCharge
$t->is($cm->getItemsCharge(),235,'ItemsCharge: '.format_currency($cm->getItemsCharge(), sfConfig::get('app_rt_currency', 'AUD')));
// SubTotal
$t->is($cm->getSubTotal(),235,'SubTotal:    '.format_currency($cm->getSubTotal(), sfConfig::get('app_rt_currency', 'AUD')));
$t->comment('-----------------------------------------------------------------------------');
// Tax
$t->is($cm->getTaxCharge(),23.50,'Tax:        '.format_currency($cm->getTaxCharge(), sfConfig::get('app_rt_currency', 'AUD')));
// Promotion
$t->is($cm->getPromotionReduction(),4,'Promotion: -'.format_currency($cm->getPromotionReduction(), sfConfig::get('app_rt_currency', 'AUD')));
// Shipping
$t->is($cm->getShippingCharge(),10,'Shipping:   '.format_currency($cm->getShippingCharge(), sfConfig::get('app_rt_currency', 'AUD')));
$t->comment('-----------------------------------------------------------------------------');
// Pre total
$t->is($cm->getPreTotalCharge(),264.50,'PreTotal:   '.format_currency($cm->getPreTotalCharge(), sfConfig::get('app_rt_currency', 'AUD')));
$t->comment('-----------------------------------------------------------------------------');
// Voucher
...
$t->comment('=============================================================================');
// Total
$t->is($cm->getTotalCharge(),254.50,'Total:      '.format_currency($cm->getTotalCharge(), sfConfig::get('app_rt_currency', 'AUD')));
$t->comment('=============================================================================');

/**
 * rtShopComplexOrder2TestTools Class
 */
class rtShopComplexOrder2TestTools
{
  /**
